...
from ai_assistant import generate_pregnancy_answer

TOKEN = os.getenv("BOT_TOKEN", "").strip()
if not TOKEN:
    raise RuntimeError("BOT_TOKEN is not set")

EXPERT_CHAT_IDS: set[int] = {
    int(x.strip())
    for x in os.getenv("EXPERT_CHAT_IDS", "").split(",")
    if x.strip().isdigit()
}
...
@dp.message(PregnancyStates.waiting_user_question)
async def process_user_question(message: types.Message, state: FSMContext):
    ...
    if ai_answer:
        await wait_msg.edit_text(
            ...
            "_Гинеколог также получила ваш вопрос и ответит в этом чате, "
            "как только освободится._",
            parse_mode="Markdown",
            reply_markup=get_question_mode_keyboard(),
        )
    else:
...
async def notify_experts_about_question(
    ...
    if not EXPERT_CHAT_IDS:
        print(f"⚠️ EXPERT_CHAT_IDS не задан — вопрос #{question_id} не отправлен экспертам")
        return
    ...
    if ai_answer:
        ...
        expert_text += f"\n🤖 **Ответ ИИ:**\n{preview}\n"
    expert_text += f"\nОтветить пользователю:\n`/reply {question_id} ваш текст`"

    for expert_id in EXPERT_CHAT_IDS:
        try:
            await bot.send_message(expert_id, expert_text, parse_mode="Markdown")
        except Exception as exc:  # noqa: BLE001
            print(f"Не удалось отправить вопрос #{question_id} эксперту {expert_id}: {exc}")
...
async def expert_reply_to_user(message: types.Message):
    ...
    if len(parts) < 3:
        await message.answer("Формат: `/reply <номер_вопроса> <текст ответа>`", parse_mode="Markdown")
        return
    try:
    ...
    if row[8] != "pending_expert":
        await message.answer(f"На вопрос #{question_id} уже дан ответ гинеколога.")
        return
