...
async def send_nav_screen_from_callback(
    ...
    return await send_nav_screen(
        ...
        reply_markup,
        parse_mode,
        also_delete=(callback.message.message_id,),
    )
...
async def show_onboarding_pick(message: types.Message, state: FSMContext) -> None:
    ...
    await _try_delete_message(message.chat.id, remove_msg.message_id)
...
async def send_first_trimester_checklist(
    ...
    *,
    ...
    if callback:
        await send_nav_screen_from_callback(
            callback, state, text, reply_markup=keyboard, parse_mode="HTML"
        )
    else:
        await send_nav_screen(message, state, text, reply_markup=keyboard, parse_mode="HTML")
