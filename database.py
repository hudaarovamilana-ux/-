...
def init_db():
    ...
    # 📨 Таблица для отслеживания отправленных ежедневных сообщений
    cursor.execute('''
    ...
    # 📊 Таблица для отслеживания отправленных сообщений за день (защита от дублей)
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS daily_sent_tracker (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER,
            date TEXT,
            schedule_type TEXT,
            message_sent INTEGER DEFAULT 0,
            UNIQUE(user_id, date, schedule_type)
        )
    ''')
...
# 📨 ФУНКЦИИ ДЛЯ ЕЖЕДНЕВНОЙ РАССЫЛКИ СООБЩЕНИЙ

def get_active_users_for_daily_messaging():
    """Получает список активных пользователей для ежедневной рассылки."""
    conn = None
    try:
        conn = get_connection()
        cursor = conn.cursor()
...
def is_message_sent_today(user_id, schedule_type):
    """Проверяет, было ли сообщение уже отправлено сегодня пользователю.
    
    Args:
        user_id: ID пользователя
        schedule_type: 'morning' или 'evening'
    
    Returns:
        bool: True если сообщение уже отправлено сегодня
    """
    conn = None
    try:
        conn = get_connection()
        ...
        cursor.execute('''
            SELECT message_sent FROM daily_sent_tracker
            WHERE user_id = ? AND date = ? AND schedule_type = ?
        ''', (user_id, today, schedule_type))
        
        result = cursor.fetchone()
        return result is not None and result[0] == 1
    except Exception as e:
        print(f"❌ Ошибка проверки отправки сообщения: {e}")
        return False
    finally:
        if conn:
            conn.close()
...
def mark_message_sent(user_id, schedule_type, message_index):
    """Отмечает, что сообщение было отправлено пользователю сегодня.
    
    Args:
        user_id: ID пользователя
        schedule_type: 'morning' или 'evening'
        message_index: индекс отправленного сообщения
    """
    conn = None
    try:
        conn = get_connection()
        ...
        # Добавляем запись в лог отправленных сообщений
        cursor.execute('''
            INSERT INTO daily_message_log
            (user_id, message_index, send_time, schedule_type, created_at)
            VALUES (?, ?, ?, ?, ?)
        ''', (user_id, message_index, now, schedule_type, now))
        
        # Отмечаем в трекере, что сообщение отправлено
        cursor.execute('''
            INSERT OR REPLACE INTO daily_sent_tracker
            (user_id, date, schedule_type, message_sent)
            VALUES (?, ?, ?, 1)
        ''', (user_id, today, schedule_type))
        
        conn.commit()
        print(f"✅ Сообщение {schedule_type} для пользователя {user_id} отмечено как отправленное")
    except Exception as e:
        print(f"❌ Ошибка отметки отправки сообщения: {e}")
    finally:
        if conn:
            conn.close()


def get_sent_message_indices_today(user_id):
    """Получает индексы сообщений, отправленных пользователю сегодня.
    
    Args:
        user_id: ID пользователя
    
    Returns:
        list: список индексов отправленных сообщений
    """
    conn = None
    try:
        conn = get_connection()
        ...
        cursor.execute('''
            SELECT DISTINCT message_index FROM daily_message_log
        ...
        print(f"❌ Ошибка получения отправленных индексов: {e}")
        return []
    finally:
        if conn:
            conn.close()